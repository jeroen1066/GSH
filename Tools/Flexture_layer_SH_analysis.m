function [V] = Flexture_layer_SH_analysis(nmax,geoid,Re,rhoE,max_bin,fupper,flower,fdens,falpha,Elastic_thickness)
%
...
%     end
    ...
    D = 200e9*(Elastic_thickness)^3/(12*(1-0.5^2));
    PHI = (1 + (D)/(500*3.72076).*(2.*(n+1)./(2*Re)).^4).^(-1);
    ...
    [Clm,Slm,llvec,mmvec] = sc2vecml(sc_flex,nmax);
